...
class Backend(object):
    ...
    def create_x509_certificate(self, builder, private_key, algorithm):
        ...
        if isinstance(algorithm, hashes.MD5) and not isinstance(
            ...
            raise ValueError("MD5 is only (reluctantly) supported for RSA certificates")
    ...
    def _create_ocsp_basic_response(self, builder, private_key, algorithm):
        ...
        if res != 1:
            ...
            self.openssl_assert(
                errors[0]._lib_reason_match(
                    self._lib.ERR_LIB_X509, self._lib.X509_R_KEY_VALUES_MISMATCH
                )
            )
    ...
    def _openssh_public_key_bytes(self, key):
        if isinstance(key, rsa.RSAPublicKey):
            ...
            raise ValueError("OpenSSH encoding is not supported for this key type")
        ...
        if res != 1:
            ...
            if not self._lib.CRYPTOGRAPHY_OPENSSL_LESS_THAN_111:
                ...
                self.openssl_assert(
                    ...
                    or errors[0]._lib_reason_match(
                        self._lib.ERR_LIB_EVP, self._lib.EVP_R_MEMORY_LIMIT_EXCEEDED
                    )
                )
